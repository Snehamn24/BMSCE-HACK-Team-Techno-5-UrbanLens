...
export default function CitizenLoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${ENV.API_BASE_URL}/auth/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: username.trim(), password }) });
      const data = await res.json();
      if (!res.ok) { showPopup('Authentication Failed', data.error || 'Invalid credentials.'); return; }
      login(data.token, data.user); router.replace('/citizen-dashboard');
      ...
      <ScrollView contentContainerStyle={s.page} keyboardShouldPersistTaps="handled">
        <Animated.View style={[s.card, glass, { opacity: fadeAnim }]}>
          <View style={s.header}>
            ...
            <Text style={s.subtitle}>Report civic issues and track resolutions</Text>
          </View>
          <Text style={s.label}>Username</Text>
          <TextInput style={[s.input, errors.username && s.inputErr]} placeholder="Enter your username" placeholderTextColor="#b0a898" autoCapitalize="none" value={username} onChangeText={v => { setUsername(v); setErrors(p => ({ ...p, username: '' })); }} />
          ...
          <TouchableOpacity style={s.switchBtn} onPress={() => router.replace('/signup')}><Text style={s.switchText}>Don't have an account? <Text style={s.switchLink}>Sign Up</Text></Text></TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  dot: { width: 10, height: 10, borderRadius: 5, backgroundColor: '#1e1e2e', marginBottom: 16 },
  ...
  button: { backgroundColor: '#1e1e2e', paddingVertical: 15, borderRadius: 12, alignItems: 'center', marginTop: 24 },
