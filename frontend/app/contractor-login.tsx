...
export default function ContractorLoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    if (!contractorId.trim()) e.contractorId = 'Required';
    if (!password) e.password = 'Required';
    setErrors(e); if (Object.keys(e).length > 0) return;
    setLoading(true);
    try {
      const res = await fetch(`${ENV.API_BASE_URL}/contractors/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contractorId: contractorId.trim(), password }) });
      const data = await res.json();
      if (!res.ok) { showPopup('Authentication Failed', data.error || 'Invalid credentials.'); return; }
      login(data.token, data.user); router.replace('/contractor-dashboard');
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={s.page} keyboardShouldPersistTaps="handled">
        <Animated.View style={[s.card, glass, { opacity: fadeAnim }]}>
          <View style={s.header}>
            <View style={[s.dot, { backgroundColor: '#4a7c59' }]} />
            <Text style={s.title}>Contractor Portal</Text>
            <Text style={s.subtitle}>Login with your assigned Contractor ID</Text>
          </View>
          <Text style={s.label}>Contractor ID</Text>
          <TextInput style={[s.input, errors.contractorId && s.inputErr]} placeholder="e.g. CTR-001" placeholderTextColor="#b0a898" autoCapitalize="characters" value={contractorId} onChangeText={v => { setContractorId(v); setErrors(p => ({ ...p, contractorId: '' })); }} />
          ...
          <TouchableOpacity style={[s.button, loading && { opacity: 0.6 }]} onPress={handleLogin} disabled={loading}>
            <Text style={s.buttonText}>{loading ? 'Authenticating...' : 'Sign In'}</Text>
          </TouchableOpacity>
      ...
      <Modal visible={popup.visible} transparent animationType="none" onRequestClose={hidePopup}>
        <View style={s.modalOv}><Animated.View style={[s.modalC, glass, { transform: [{ scale: popupScale }] }]}>
          ...
          <TouchableOpacity style={[s.modalBtn, { backgroundColor: '#4a7c59' }]} onPress={hidePopup}><Text style={s.modalBtnT}>Dismiss</Text></TouchableOpacity>
        </Animated.View></View>
      </Modal>
...
const s = StyleSheet.create({
  ...
  subtitle: { fontSize: 13, color: '#8b7e6a', marginTop: 4, textAlign: 'center' },
  ...
  button: { backgroundColor: '#4a7c59', paddingVertical: 15, borderRadius: 12, alignItems: 'center', marginTop: 24 },
